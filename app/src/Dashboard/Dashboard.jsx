...
const SidebarItem = ({ icon, label, active, onClick }) => (
  ...
  >
    <span className={`transition-colors duration-200 ${active ? 'text-white' : 'text-slate-400 group-hover:text-[#2E8B57]'}`}>
      {icon}
    </span>
...
const Dashboard = ({ onLogout }) => {
      ...
      <div className="animate-in fade-in duration-500">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-slate-500 text-sm">System administration and data management</p>
          </div>
          <div className="flex space-x-3">
            <button className="bg-white border border-slate-200 text-slate-600 px-4 py-2 rounded-xl text-sm font-semibold hover:bg-slate-50 transition-colors">Export Report</button>
            <button className="bg-[#2E8B57] text-white px-4 py-2 rounded-xl text-sm font-semibold hover:bg-[#257045] transition-colors shadow-lg shadow-[#2E8B57]/20">+ Add New Entry</button>
          </div>
        </div>
        ...
        {activeMenu === 'Dashboard Home' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
              ...
              <p className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-1">Total Victims</p>
              <h4 className="text-3xl font-extrabold text-slate-800">4,281</h4>
              <div className="mt-2 text-emerald-500 text-xs font-bold flex items-center">
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
              ...
              <p className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-1">Pending Cases</p>
              <h4 className="text-3xl font-extrabold text-slate-800">156</h4>
              <div className="mt-2 text-amber-500 text-xs font-bold flex items-center">
                <svg className="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M5 10a1 1 0 011-1h8a1 1 0 110 2H6a1 1 0 01-1-1z" clipRule="evenodd" /></svg>
                Active monitoring
              </div>
            </div>
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
              ...
              <p className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-1">High Risk</p>
              <h4 className="text-3xl font-extrabold text-slate-800">42</h4>
              <div className="mt-2 text-red-500 text-xs font-bold flex items-center">
                Requires immediate action
              </div>
            </div>
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
              <div className="w-12 h-12 bg-[#E0F2F1] text-[#2E8B57] rounded-xl flex items-center justify-center mb-4">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
              </div>
              <p className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-1">Resolved Cases</p>
              <h4 className="text-3xl font-extrabold text-slate-800">3,109</h4>
              <div className="mt-2 text-emerald-500 text-xs font-bold flex items-center">
                Success rate 94%
              </div>
            </div>
            ...
            <div className="lg:col-span-4 bg-white p-8 rounded-3xl border border-slate-100 shadow-sm mt-6">
               <h3 className="text-lg font-bold text-slate-800 mb-6">Recent Investigation Logs</h3>
                 ...
                 <table className="w-full text-left">
                   <thead>
                     <tr className="text-slate-400 text-xs font-bold uppercase tracking-wider border-b border-slate-50">
                       ...
                       <th className="pb-4">Victim Name</th>
                   ...
                   <tbody className="text-sm">
                     {[1,2,3,4,5].map(i => (
                       <tr key={i} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                         <td className="py-4 px-2 font-mono text-xs text-[#2E8B57]">#SG-2024-00{i}</td>
                         <td className="py-4 font-bold text-slate-700">Victim_{i*100}</td>
                         <td className="py-4 text-slate-500">Physical Abuse</td>
                         <td className="py-4">
                           <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase ${i % 2 === 0 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
                             {i % 2 === 0 ? 'Pending' : 'In Progress'}
                           </span>
                         </td>
                         <td className="py-4 text-slate-400">2 hours ago</td>
                         <td className="py-4 text-right">
                           <button className="text-[#2E8B57] font-bold text-xs hover:underline">View Details</button>
                         </td>
                       </tr>
                     ))}
                   </tbody>
          ...
          <div className="bg-white p-20 rounded-3xl border border-slate-100 shadow-sm flex flex-col items-center justify-center text-center">
             ...
             <h3 className="text-xl font-bold text-slate-800 mb-2">{activeMenu} Management</h3>
             <p className="text-slate-500 max-w-sm">This module allows for the configuration and oversight of all {activeMenu.toLowerCase()} related records in the central database.</p>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans overflow-hidden">
      ...
      {/* Sidebar */}
      <aside 
        className={`${sidebarCollapsed ? 'w-20' : 'w-72'} bg-white border-r border-slate-100 flex flex-col transition-all duration-300 relative z-30 shadow-sm overflow-hidden`}
      >
        <div className="p-6 flex items-center space-x-3 mb-4">
          ...
          {!sidebarCollapsed && <span className="text-xl font-bold text-slate-800 tracking-tight whitespace-nowrap">SafeGuard <span className="text-[#2E8B57]">DBMS</span></span>}
        </div>
      ...
      <div className="flex-grow flex flex-col min-w-0">
        ...
        {/* Navbar */}
        <header className="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-8 relative z-20">
          <div className="flex items-center space-x-4">
            <div className="relative group">
              ...
              <input 
                ...
                className="pl-10 pr-4 py-2 bg-slate-50 border border-slate-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#2E8B57]/20 transition-all w-64 md:w-96"
              />
            </div>
          </div>
          ...
          <div className="flex items-center space-x-6">
            ...
            <div className="relative">
              <button 
                ...
                className="w-10 h-10 rounded-xl flex items-center justify-center text-slate-400 hover:bg-slate-50 hover:text-[#2E8B57] transition-all relative"
              >
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>
                <span className="absolute top-2 right-2.5 w-2 h-2 bg-red-500 rounded-full border-2 border-white"></span>
              </button>
              {isNotificationsOpen && (
                <div className="absolute right-0 mt-4 w-80 bg-white rounded-2xl shadow-2xl border border-slate-100 p-4 animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-50">
                    <h5 className="font-bold text-slate-800">Notifications</h5>
                    <button className="text-[10px] font-bold text-[#2E8B57] uppercase">Mark all read</button>
                  </div>
                  <div className="space-y-3">
                    <div className="p-2 hover:bg-slate-50 rounded-lg transition-colors cursor-pointer">
                      ...
                      <p className="text-[10px] text-slate-500">Requires urgent field assessment</p>
                    </div>
                    <div className="p-2 hover:bg-slate-50 rounded-lg transition-colors cursor-pointer border-l-2 border-[#2E8B57]">
                      <p className="text-xs font-bold text-slate-800">System Update</p>
                      <p className="text-[10px] text-slate-500">Security patches applied successfully</p>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="relative">
              <button 
                ...
                className="flex items-center space-x-3 hover:bg-slate-50 p-2 rounded-2xl transition-all"
              >
                <div className="text-right hidden md:block">
                  <p className="text-sm font-bold text-slate-800 leading-none">{user.name}</p>
                  <p className="text-[10px] text-slate-400 font-bold uppercase mt-1 tracking-wider">{user.role}</p>
                </div>
                <img src={user.avatar} className="w-10 h-10 rounded-xl border-2 border-[#2E8B57]/10" alt="Avatar" />
              </button>
              {isProfileOpen && (
                <div className="absolute right-0 mt-4 w-64 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="p-6 border-b border-slate-50 bg-slate-50/50">
                    <p className="text-xs font-bold text-slate-400 uppercase mb-1">Signed in as</p>
                    <p className="text-sm font-bold text-slate-800 truncate">{user.email}</p>
                  </div>
                  <div className="p-2">
                    <button className="w-full flex items-center space-x-3 px-4 py-2.5 rounded-xl text-slate-600 hover:bg-slate-50 hover:text-[#2E8B57] transition-all">
