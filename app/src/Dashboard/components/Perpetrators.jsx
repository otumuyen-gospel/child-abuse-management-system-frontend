...
const Perpetrators = ({ menuItems, activeMenu }) => {
    ...
    return (
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-slate-500 text-xs md:text-sm">List Of Perpetrators</p>
          </div>
          <div className="flex w-full sm:w-auto space-x-2 md:space-x-3">
            <button onClick={()=>handleOpenMessageBox(true, false, "Export perpetrator", 
            "info", "This action will create an excel record on all the perpetrator record","Cancel", "OK")} 
            className="flex-1 sm:flex-none bg-white border border-slate-200 text-slate-600 px-3 md:px-4 py-2 rounded-xl text-xs md:text-sm font-semibold hover:bg-slate-50 transition-colors">
              Export</button>
            <button onClick={()=>handleOpenDialogBox(true, false, "New Perpetrator", "Please Enter Details of The Perpetrator")}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            <div className="lg:col-span-4 bg-white p-5 md:p-8 rounded-3xl border border-slate-100 shadow-sm mt-6 overflow-hidden">
               <h3 className="text-base md:text-lg font-bold text-slate-800 mb-6">Perpetrator Records</h3>
                 ...
                 <table className="w-full text-left min-w-[600px]">
                   <thead>
                     <tr className="text-slate-400 text-[10px] md:text-xs font-bold uppercase tracking-wider border-b border-slate-50">
                       <th className="pb-4 px-2">Perpetrator ID</th>
                       <th className="pb-4">Person ID</th>
                       <th className="pb-4">Contact ID</th>
                       <th className="pb-4">Relationship To Victim</th>
                       <th className="pb-4">Prior Offenses</th>
                   ...
                   <tbody className="text-xs md:text-sm">
                     {[1,2,3,4,5].map(i => (
                       <tr key={i} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                         ...
                         <td className="py-4 text-slate-500">#2345</td>
                         <th className="pb-4">Uncle</th>
                         <th className="pb-4">Physical Abuse</th>
                          <td className="py-4 text-right">
                           <button onClick={()=>handleOpenDialogBox(false, true, "Update Perpetrator", "Please Enter New Details Below")}
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 mt-4">
               ...
               children={
                <div className="space-y-6 py-2">
                   <div className="space-y-1.5">
                       <label htmlFor="person" className="text-xs font-bold uppercase tracking-widest text-slate-400 pl-1">
                        ...
                        required className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-[#2E8B57]/20 transition-all font-medium text-slate-900 appearance-none">
                           <option value="" selected disabled>Select Perpetrator</option>
                   ...
                   <div className="space-y-1.5">
                       <label htmlFor="contact" className="text-xs font-bold uppercase tracking-widest text-slate-400 pl-1">
                        Contact</label>
                        <select name="contact"
                        required className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-[#2E8B57]/20 transition-all font-medium text-slate-900 appearance-none">
                    ...
                    <div className="space-y-1.5">
                       <label htmlFor="relationship" className="text-xs font-bold uppercase tracking-widest text-slate-400 pl-1">
                        Relationship</label>
                   ...
                   <div className="space-y-1.5">
                       <label htmlFor="offenses" className="text-xs font-bold uppercase tracking-widest text-slate-400 pl-1">
                        Previous Offenses</label>
                        <textarea name="offenses" rows={10} placeholder="Enter offenses"
                        required className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-[#2E8B57]/20 transition-all font-medium text-slate-900 appearance-none"/>
                   </div>
