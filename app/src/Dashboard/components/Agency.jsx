...
const Agency = ({ menuItems, activeMenu }) => {
    ...
    return (
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-slate-500 text-xs md:text-sm">Child Management Agency</p>
          </div>
          <div className="flex w-full sm:w-auto space-x-2 md:space-x-3">
            <button onClick={()=>handleOpenMessageBox(true, false, "Export agency", 
            "info", "This action will create an excel record on all the agencies record","Cancel", "OK")} 
            className="flex-1 sm:flex-none bg-white border border-slate-200 text-slate-600 px-3 md:px-4 py-2 rounded-xl text-xs md:text-sm font-semibold hover:bg-slate-50 transition-colors">
              Export</button>
            <button onClick={()=>handleOpenDialogBox(true, false, "New Agency", "Please Enter Details of Agency")}
                 ...
                 <table className="w-full text-left min-w-[600px]">
                   <thead>
                     <tr className="text-slate-400 text-[10px] md:text-xs font-bold uppercase tracking-wider border-b border-slate-50">
                       <th className="pb-4 px-2">Agency ID</th>
                       <th className="pb-4">Agency Name</th>
                       <th className="pb-4">Agency Description</th>
                       <th className="pb-4">Agency Address</th>
                       <th className="pb-4">Agency Phone</th>
                   ...
                   <tbody className="text-xs md:text-sm">
                     {[1,2,3,4,5].map(i => (
                       <tr key={i} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                         ...
                         <td className="py-4 font-bold text-slate-700">SafeGaurd DBMS</td>
                         <td className="py-4 text-slate-500">SafeGaurd is a child protection management agency</td>
                         <th className="pb-4">Agency Address</th>
                         <th className="pb-4">Agency Phone</th>
                          <td className="py-4 text-right">
                           <button onClick={()=>handleOpenDialogBox(false, true, "Update Agency", "Please Enter New Details Below")}
                ...
                <div className="space-y-6 py-2">
                   <div className="space-y-1.5">
                       ...
                       <input 
                           ...
                           placeholder="e.g. SafeGaurd DBMS" 
                        />
                   </div>
            
                   <div className="space-y-1.5">
                          <label htmlFor="Description" className="text-xs font-bold uppercase tracking-widest text-slate-400 pl-1">
                              Description
                          </label>
                          <textarea 
                              id="Description"
                              rows="5"
                              defaultValue="SageGaurd is a child protection management agency"
                              ...
                              placeholder="SageGaurd is a child protection management agency"
                          />
                    </div>
                    ...
                    <div className="space-y-1.5">
                          ...
                          <input 
                              id="Address"
                              type="text"
                              defaultValue="plymouth, 37A GrillWay"
                              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-[#2E8B57]/20 transition-all resize-none font-medium text-slate-900 leading-relaxed" 
                              placeholder="SageGaurd office Address"
                          />
                    </div>
