...
const Screening = ({ menuItems, activeMenu }) => {
    ...
    return (
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-slate-500 text-xs md:text-sm">List Of Screening Records</p>
          </div>
          <div className="flex w-full sm:w-auto space-x-2 md:space-x-3">
            <button onClick={()=>handleOpenMessageBox(true, false, "Export screening", 
            "info", "This action will create an excel record on all the screening records","Cancel", "OK")} 
            className="flex-1 sm:flex-none bg-white border border-slate-200 text-slate-600 px-3 md:px-4 py-2 rounded-xl text-xs md:text-sm font-semibold hover:bg-slate-50 transition-colors">
              Export</button>
            <button onClick={()=>handleOpenDialogBox(true, false, "New Screening", "Please Enter Details of The Screening Record")}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            <div className="lg:col-span-4 bg-white p-5 md:p-8 rounded-3xl border border-slate-100 shadow-sm mt-6 overflow-hidden">
               <h3 className="text-base md:text-lg font-bold text-slate-800 mb-6">Screening Records</h3>
                 ...
                 <table className="w-full text-left min-w-[600px]">
                   <thead>
                     <tr className="text-slate-400 text-[10px] md:text-xs font-bold uppercase tracking-wider border-b border-slate-50">
                       <th className="pb-4 px-2">Screening ID</th>
                       <th className="pb-4">report ID</th>
                       <th className="pb-4">Outcome</th>
                       <th className="pb-4">Approval Status</th>
                   ...
                   <tbody className="text-xs md:text-sm">
                     {[1,2,3,4,5].map(i => (
                       <tr key={i} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                         ...
                         <td className="py-4 text-slate-500">outcome</td>
                         <th className="pb-4">status</th>
                          <td className="py-4 text-right">
                           <button onClick={()=>handleOpenDialogBox(false, true, "Update Screening", "Please Enter New Details Below")}
                ...
                <div className="space-y-6 py-2">
                   ...
                   <div className="space-y-1.5">
                       <label htmlFor="outcome" className="text-xs font-bold uppercase tracking-widest text-slate-400 pl-1">
                        Outcome</label>
                        <textarea name="outcome" placeholder="Enter outcome"
                        required className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-[#2E8B57]/20 transition-all font-medium text-slate-900 appearance-none"/>
                   </div>
                   ...
                   <div className="space-y-1.5">
                       ...
                       Approval Status</label>
                        <input name="status" type="text" placeholder="Enter approval status"
                        required className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-[#2E8B57]/20 transition-all font-medium text-slate-900 appearance-none"/>
                   </div>
